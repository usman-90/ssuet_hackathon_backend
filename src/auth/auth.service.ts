...
import { SignUpDto } from './dtos/requestDtos/signup.dto'
...
export class AuthService {
  ...
  async userSignUp(signUpDto: SignUpDto): Promise<SignUpResponseDto> {
    const isEmailAvailable =
      (await this.user_service.find_one_by_email(signUpDto.email)) ? false : true
    ...
    try {
      ...
      const user = await this.user_service.insert(signUpDto)
  ...
  async verifyEmailByOtp(email: string, otp: string): Promise<VerifyEmailByOtpResponse> {
  ...
  async reset_password(
    ...
    try {
      if (!reset_password_token) throw new UnauthorizedException('not Authorized')

      const user_password_payload: ResetPasswordPayload = await this.jwtService.verifyAsync(
        reset_password_token,
        {
      ...
      if (user_password_payload.email !== email || !user_password_payload.allow_reset) {
